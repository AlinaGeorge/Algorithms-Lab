...
int CoinChange(int n, int D[], int m){
	int F[n+1];
    int track[n+1];  // Array to track coins used
	F[0] = 0;
	...
	for(int i = 1; i <= n; i++){
		...
		while(j < m && i >= D[j]){
			if(F[i - D[j]] < temp){
				temp = F[i - D[j]];
				chosenCoin = j;  // Track the coin index used
			}
			j++;
		}
		F[i] = temp + 1;
		track[i] = chosenCoin;  // Store which coin was used to form amount i
	}
	
	display(F, n);
	displayCoins(D, n, track);  // Display the coin combination
